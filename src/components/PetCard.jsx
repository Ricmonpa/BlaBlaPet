  ...
  useEffect(() => {
    ...
    if (video && post.mediaType === 'video') {
      ...
      const handleError = (e) => {
        ...
        // Verificar si es un video de Vercel Blob (ya no funcionan después de la migración)
        const isVercelBlob = video.src && video.src.includes('blob.vercel-storage.com');
        
        if (isVercelBlob) {
          console.log('🚫 Video de Vercel Blob detectado (migrado a Cloudinary), ocultando');
          video.style.display = 'none';
        ...
        {post.mediaType === 'video' ? (
          <video
            ...
            onError={(e) => {
              ...
              // Verificar si es una URL blob expirada o de Vercel Blob
              const isBlobUrl = e.target.src && e.target.src.startsWith('blob:');
              const isVercelBlobUrl = e.target.src && e.target.src.includes('blob.vercel-storage.com');
              if (isBlobUrl || isVercelBlobUrl) {
                console.log('🚫 URL expirada o de Vercel Blob en onError, ocultando video');
              }
